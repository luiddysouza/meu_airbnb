...
void main() {
  ...
  group('ConectividadeChannel', () {
    group('ConectividadeChannel obterStatusStream', () {
      test('deve retornar um Stream<String> em qualquer plataforma', () {
        final stream = ConectividadeChannel.obterStatusStream();
        expect(stream, isA<Stream<String>>());
      });

      test('deve não lançar exceção ao escutar eventos', () async {
        final stream = ConectividadeChannel.obterStatusStream();
        expect(() => stream.listen((_) {}), isNot(throwsException));
      });

      test('deve emitir eventos de tipo String', () async {
        final stream = ConectividadeChannel.obterStatusStream();
      ...
      test('deve permitir múltiplos listeners (broadcast stream)', () async {
        final stream = ConectividadeChannel.obterStatusStream();
    ...
    group('ConectividadeChannel obterStatusAtual', () {
      test('deve retornar Future<String> em qualquer plataforma', () {
    ...
    group('ConectividadeChannel Comportamento cross-platform', () {
      test('Stream deve ser robusta contra PlatformException', () async {
        final stream = ConectividadeChannel.obterStatusStream();
