...
void main() {
  ...
  group('ShareChannel', () {
    group('compartilharHospedagem', () {
      test('deve retornar false em plataformas sem suporte (web/iOS)', () async {
        // Act
      ...
      test('deve aceitar todos os parâmetros sem lançar exceção', () async {
        // Act
        final resultado = await ShareChannel.compartilharHospedagem(
          titulo: 'Casa',
          descricao: 'Descrição',
      ...
      test('deve aceitar URL nula sem lançar exceção', () async {
        // Act
        final resultado = await ShareChannel.compartilharHospedagem(
          titulo: 'Casa',
          descricao: 'Descrição',
    ...
    group('compartilharLista', () {
      ...
      test('deve aceitar lista vazia sem lançar exceção', () async {
        // Act
        final resultado = await ShareChannel.compartilharLista(
          titulo: 'Vazio',
          hospedagens: const [],
      ...
      test(
        ...
        () async {
          // Arrange
          final hospedagens = [
            {'nome': 'Casa 1', 'valor': 'R\$ 200', 'cidade': 'São Paulo'},
            {'nome': 'Casa 2', 'valor': 'R\$ 300', 'cidade': 'Rio de Janeiro'},
            {'nome': 'Casa 3', 'valor': 'R\$ 150'},
          ];
      ...
      test('deve fornecer documentação de uso no código', () {
    ...
    group('Comportamento em diferentes plataformas', () {
      test('não deve lançar exceção em nenhuma plataforma', () async {
        // Arrange
        const titulo = 'Casa';
      ...
      test('deve sempre retornar um booleano', () async {
        // Act
